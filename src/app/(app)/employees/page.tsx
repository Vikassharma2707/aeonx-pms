...
const EMPTY_FORM: EmployeeFormData = {
  ...
  line_manager: '', project_manager: '', hire_date: '', employment_type: '',
  active_status: 'Active', appraisal_band: '', email: '',
}
...
export default function EmployeesPage() {
  ...
  function openEdit(emp: Employee) {
    ...
    setFormData({
      ...
      practice: emp.practice ?? '', location: emp.location ?? '',
      line_manager: emp.line_manager ?? '', project_manager: (emp as Employee & { project_manager?: string }).project_manager ?? '',
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-200">
                  {['Employee ID','Name','Designation','Practice','Location','Line Manager','Project Manager','Employment Type','Status','Appraisal Band','Actions'].map(h => (
                ...
                {loading ? (
                  ...
                  filtered.map((emp) => (
                    <tr key={emp.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-gray-600 whitespace-nowrap">{(emp as Employee & { project_manager?: string }).project_manager ?? '-'}</td>
      ...
      <Modal isOpen={modalOpen} onClose={closeModal} title={editingEmployee ? 'Edit Employee' : 'Add Employee'} size="lg">
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <FormField label="Line Manager (Employee ID)"><Input placeholder="e.g. 96000100" value={formData.line_manager} onChange={(e) => handleFormChange('line_manager', e.target.value)} /></FormField>
            <FormField label="Project Manager (Employee ID)"><Input placeholder="e.g. 96000200" value={formData.project_manager} onChange={(e) => handleFormChange('project_manager', e.target.value)} /></FormField>
